...
- **Runtime, agent loop & protocol** (see [`docs/architecture.md`](docs/architecture.md) for the full picture): every agent is an actor with an inbox, driven by the runtime pump (`src/runtime/`). **Every agent — leaf or orchestrator — runs the same event-driven loop** (`on_start → _advance → _think → _dispatch_round → _run_one → _conclude`; a "round" == one turn's concurrent batch of tool calls). `MetaAgent` is a normal `Agent` that only flips `_include_agents=True` (sub-agents become callable tools) and handles escalations — a **sub-agent is just another capability** it dispatches. Agent-to-agent conversations go through **`protocol_manager`** (`src/protocol/`) — escalation / delegation / progress / control / query / pubsub — which is layered on the runtime's transport verbs (`send` / `ask` / `suspend`+`resume` / `publish`+`subscribe`).
