...
| **一步是一次决策，不是一次等待** | 耗时命令、往终端里的输入、派出去的子智能体都可以放到后台启动，之后凭 job id 取结果；到点的提醒也记在同一份登记里。一步花在等待上，这一步的决定就等于没发生过。 |
| **状态不随调用一起消失** | 终端在两次调用之间保持存活：切过的目录、激活的环境、连上的 ssh、开着的 REPL 都还在。可续的后台子智能体保留自己的会话，可以继续派新活。读过的图片会重新附到后续请求上，而不是随读它的那一步一起消失。 |
| **代码模式** | 模型可以直接写一段程序，而不是一轮只发一次调用；程序里的调用经受控派发桥接回来，一批工具工作只花一轮。程序跑在自己的解释器里：框架对象不会离模型写的代码只有一个 `import` 的距离。 |
| **计划模式** | 人可以让一次运行先停在读和想的阶段，等自己认可它打算做的事之后再放行。闸门只看能力自己声明的 `mutates` / `permission_mode`，不看名字；两者都没声明的能力一律拦下。 |
| **崩溃安全续跑** | 结构化、可移植的执行 checkpoint 以原子方式写入。运行中断后，浏览器和 CLI 都会展示恢复状态；无法安全自动续跑时，必须明确选择继续或重新开始。 |
| **统一审批与生命周期入口** | Tool、Connector 与 Environment Action 共用同一套副作用分类和审批路径，未声明副作用时默认收紧。类型化 Hook 总线统一发布 prompt、session、task、能力调用和 rollout 事件，同时避免把横切逻辑耦合进 Agent 循环。 |
| **项目记忆与实时 Agent 线程** | Project Memory 跨 session 保留；被委派 Agent 作为可寻址的运行线程继续支持直接消息、进度、控制与后续任务。阻塞调用结束后会及时释放 driver 和上下文资源。 |
| **渐进发布与可观测性** | 扩展可经过 shadow、canary 阶段，健康证据持久化并支持自动回滚。Trace 仍是权威事实源，同时可选地通过 OpenTelemetry 导出成对 span。 |
...
| **可被缓存的提示词前缀** | 能力目录在首次渲染后就冻结下来，放在易变的 agent 状态之前，缓存断点落在它们之后；这样会话中途生成的新组件不会让它后面的整段对话作废。 |
...
这些能力大多由模型自己调用工具来使用：起一个后台任务、过一会儿再来收结果，开一个终端并往里输入，
把子智能体放到后台、之后再取回它的答案，问语言服务器某个符号定义在哪，看一张图，检索以前的运行
试过什么。[`docs/tool-catalog.md`](docs/tool-catalog.md) 由注册表实时生成，逐个列出它们的参数与
权限模式。
...
对话区呈现的是*此刻成立*的事，而不只是说过的话：这次会话要达成的目标固定在对话流上方，不随记录
一起滚走；再往上一栏是后台任务的计数，点开就是还在跑和快到点的清单；计划模式的开关贴着输入框，
因为它决定的正是下一条消息能做什么。同一栏里还能把对话中的任何一次运行摊开成一条 trajectory——
每一步的推理、参数、结果与报错、耗时、token 用量，以及提示词中来自缓存的比例。
...
- `session/query/` 在运行结束后把这些 trace 日志读回来，让 Agent 能检索以前的运行试过什么，并翻看命中处前后的步骤；
- `memory/` 维护近期历史、压缩后的工作记忆、todo、调用路径与最终结果；
- `spill/` 把过大的工具输出整份写进文件，并把位置写在摘要里，装不下的那部分仍然读得到；
- `benchmark/` 提供 AIME、GPQA、GSM8K、HLE、LeetCode、DeepWeb、ProgramBench 等评测入口。

输入 token 是最主要的重复开销，因此提示词是按缓存来排布的：能力目录每一步都逐字节相同，被放在
易变的 agent 状态之前，缓存断点落在它们之后。实测的一次运行里，编排智能体此前一个 token 都读不
回来，现在每一步约 98,800 个输入 token 中有 72,647 个来自缓存。这样安排的理由记在
`agentevolver/model/types.py` 和 `agentevolver/agent/types.py` 里，包括缓存为什么按一小时计，
以及会话中途新增的能力为什么写在目录之后、而不是改写目录本身。
...
| [模块手册](https://dvampire.github.io/AgentEvolver/modules.html) | 全部 48 个模块的可搜索、可展开参考：运行位置、公共 API 与源码入口 |
