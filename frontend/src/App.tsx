...
import { ArrowUp, ArrowUpRight, LayoutDashboard, Search, ChevronDown, Blocks, Boxes, Cable, Code2, FlaskConical, Globe, GraduationCap, Hand, MessageSquare, Monitor, MonitorPlay, Moon, PanelLeftClose, PanelLeftOpen, Paperclip, Pencil, Plug, Plus, RefreshCw, Settings, Sparkles, SquareTerminal, Sun, Waypoints, Workflow, Wrench, type LucideIcon } from 'lucide-react';

import AlertDisplayArea from './alerts';
import { ProjectOverview, type WorkspaceView } from './workspace/ProjectOverview';
...
export function App() {
  ...
  const loadLiveAgents = useCallback(async () => {
    ...
    if (sessionRef.current !== currentSessionId || !response.ok || !Array.isArray(response.result.agents)) return;
    const live = response.result.agents.flatMap((value): AgentState[] => {
      ...
      const mappedStatus: AgentState['status'] = ['new', 'running', 'idle', 'suspended', 'stopping'].includes(runtimeStatus)
        ? 'running'
  ...
  const startSession = useCallback(async (socket: GatewaySocket) => {
    try {
      ...
      if (resume) {
        ...
        // A restored session ID may already be React's initial value, so the
        // session-change effect does not run again after the socket connects.
        await Promise.all([hydrateCapabilities(socket, resume.session_id), refreshSessions(socket), loadModels(socket), loadDeploys(socket), loadHosts(socket), loadAttachments(socket, resume.session_id), loadExtensionStage(socket, resume.session_id), loadWorkspaceDirectory()]);
  ...
  }, [hydrateCapabilities, loadAttachments, loadExtensionStage, loadModels, loadDeploys, loadHosts, loadWorkspaceDirectory, refreshSessions, resumeChat]);
  ...
  const createNewSession = async () => {
    ...
    if (current && current.has_work === false) {
      setMainView('chat');
      setNotice('Describe what you want to work on.');
      return;
    }
    try {
      sessionRef.current = undefined;
  ...
  const selectSession = async (nextSession: SessionSummary) => {
    ...
    try {
      ...
      if (sessionRef.current !== nextSession.session_id) return;
      setActiveTaskId(undefined);
      const chatEvents = await resumeChat(socket, nextSession.session_id);
      if (sessionRef.current !== nextSession.session_id) return;
      for (const event of chatEvents) handleGatewayEvent(event);
      setMainView('overview');
    } catch (error) {
      setNotice(error instanceof Error ? error.message : String(error));
    }
  ...
  const promoteStagedExtension = async (reviewedStage?: ExtensionStage) => {
    const stage = reviewedStage ?? extensionStage;
    const socket = socketRef.current;
    const currentSessionId = sessionRef.current;
    if (!socket || !currentSessionId || !stage.components.length) return;
    if (!stage.valid) {
      setNotice(stage.error ?? 'Fix staged extension validation errors before promotion.');
      return;
    }
    if (!window.confirm(`Promote ${stage.components.length} staged extension component(s) to the shared extension root? Existing files will not be overwritten.`)) return;
    try {
      const relativePaths = stage.components.flatMap(item => isRecord(item) && typeof item.relative_path === 'string' ? [item.relative_path] : []);
      if (relativePaths.length !== stage.components.length) throw new Error('Reload the candidate list before promoting.');
      const response = await socket.request('extension.promote', { session_id: currentSessionId, relative_paths: relativePaths });
  ...
  return (
    ...
    <main className={`app-shell${mainView !== 'chat' ? ' canvas-wide' : ''}${sidebarCollapsed ? ' sidebar-collapsed' : ''}`} data-theme={theme} style={{ ['--sidebar-w' as string]: `${sidebarWidth}px`, ['--inspector-w' as string]: `${inspectorWidth}px` }}>
      ...
      <aside className="sidebar">
        ...
        <button className="project-switcher" onClick={() => { setProjectSearch(''); setProjectsOpen(true); }} disabled={status !== 'connected'}>
          <span><small>Current project</small><strong>{sessions.find(item => item.session_id === sessionId)?.name || 'Select a project'}</strong><code>{sessionId?.slice(0, 8) || 'Connecting…'}</code></span><ChevronDown size={15} />
        </button>
        <nav className="sidebar-section capability-nav view-nav" aria-label="Views">
          <p className="eyebrow">Workspace</p>
          <button className={mainView === 'overview' ? 'view-active' : ''} onClick={() => setMainView('overview')}><span><LayoutDashboard size={16} strokeWidth={1.9} /></span><strong>Overview</strong></button>
        ...
        <div className="sidebar-section recent-projects">
          <p className="eyebrow">Recent projects <button onClick={() => { setProjectSearch(''); setProjectsOpen(true); }}>View all</button></p>
          {projects.flatMap(([, items]) => items).slice(0, 4).map(project => <button className={`project-session ${project.session_id === sessionId ? 'selected' : ''}`} key={project.session_id} title={`${project.name} · ${project.session_id}`} onClick={() => void selectSession(project)}><span className="session-dot" /><span>{project.name}</span><em>{project.session_id.slice(0, 4)}</em></button>)}
        </div>
        </div>
        ...
        <details className="sidebar-section capability-disclosure"><summary><span><Blocks size={16} /> Capability library</span><ChevronDown size={13} /></summary>
        <nav className="capability-nav" aria-label="Capabilities">
          <p className="sidebar-hint">Available in the library</p>
          {CAPABILITY_KINDS.map((kind) => <button key={kind} onClick={() => { setActiveCapability(kind); setCapabilitySearch(''); setCapabilitiesOpen(true); }}><span><CapIcon kind={kind} /></span><strong>{CAPABILITY_META[kind].label}</strong><em>{catalog[kind].length}</em></button>)}
          <button onClick={() => { setMainView('overview'); }}><span><LayoutDashboard size={16} /></span><strong>All eight entity types</strong></button>
        </nav></details>
...
>
      ...
      {mainView === 'overview' ? <section className="conversation overview-mode">
        <header className="topbar"><div className="header-title"><button className="mobile-menu" onClick={() => setMobileNavOpen(true)} aria-label="Open navigation">☰</button><span className="overview-breadcrumb">Workspace / Overview</span></div><div className="connection"><span className={`connection-dot ${status}`} />{statusText}</div></header>
        <ProjectOverview key={sessionId || 'connecting'} request={gatewayRequest} sessionId={sessionId} name={sessions.find(item => item.session_id === sessionId)?.name || 'Your workspace'} connected={status === 'connected'} activeTaskId={activeTaskId}
          capabilities={CAPABILITY_KINDS.filter(kind => kind !== 'commands' && kind !== 'canvas').map(kind => ({ kind, available: catalog[kind].length, enabled: selection[kind].length }))}
          agents={agents} onView={setMainView} onCapability={(kind) => { setActiveCapability(kind); setCapabilitySearch(''); setCapabilitiesOpen(true); }} onAgent={setSelectedAgentId}
          onFile={(path) => { setMainView('chat'); void openWorkspaceFile(path); }} onPromote={promoteStagedExtension} />
        ...
        <header className="topbar">
          <div className="header-title"><button className="mobile-menu" onClick={() => setMobileNavOpen(true)} aria-label="Open navigation">☰</button><div><p className="eyebrow">Workspace agent</p><h1>{sessions.find(item => item.session_id === sessionId)?.name || 'Chat'}</h1></div></div>
          <div className="connection">{extensionStage.components.length ? <button className="stage-action" onClick={() => setMainView('overview')}>Review {extensionStage.components.length} candidates</button> : null}<span className={`connection-dot ${status}`} />{statusText}<button onClick={() => setSettingsOpen(true)}>Configure</button></div>
        </header>
        <div className="message-list">
          {environmentView ? <EnvironmentLive view={environmentView} onClose={() => setEnvironmentView(undefined)} /> : null}
          {!timeline.some(item => item.type === 'activity' || item.value.type === 'user' || item.value.type === 'assistant') ? <QuickStart onSelect={setDraft} /> : null}
      ...
      {projectsOpen ? <ProjectsDialog projects={projects.flatMap(([, items]) => items)} sessionId={sessionId} search={projectSearch} onSearch={setProjectSearch} onClose={() => setProjectsOpen(false)} onSelect={(project) => { void selectSession(project); setProjectsOpen(false); }} /> : null}
      {mobileNavOpen ? <MobileNavigation mainView={mainView} onView={setMainView} onProjects={() => { setProjectSearch(''); setProjectsOpen(true); }} catalog={catalog} agents={agents} status={status} theme={theme} onClose={() => setMobileNavOpen(false)} onCreateSession={createNewSession} onOpenCapabilities={(kind) => { setActiveCapability(kind); setCapabilitySearch(''); setCapabilitiesOpen(true); }} onToggleTheme={() => setTheme((current) => current === 'dark' ? 'light' : 'dark')} onOpenConnection={() => setSettingsOpen(true)} /> : null}
    </main>
    </TooltipProvider>
  );
}

function ProjectsDialog({ projects, sessionId, search, onSearch, onSelect, onClose }: {
  projects: SessionSummary[]; sessionId?: string; search: string;
  onSearch: (value: string) => void; onSelect: (project: SessionSummary) => void; onClose: () => void;
}) {
  const matches = projects.filter(project => `${project.name} ${project.session_id}`.toLowerCase().includes(search.trim().toLowerCase()));
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}><DialogContent className="project-picker-dialog">
    <DialogTitle>Find a project</DialogTitle><DialogDescription>Search by name or session ID. Your most recently updated projects appear first.</DialogDescription>
    <label className="project-picker-search"><Search size={16} /><input autoFocus aria-label="Search projects" placeholder="Search projects…" value={search} onChange={event => onSearch(event.target.value)} /></label>
    <div className="project-picker-results">{matches.length ? matches.map(project => <button key={project.session_id} className={project.session_id === sessionId ? 'selected' : ''} onClick={() => onSelect(project)}><span><strong>{project.name || 'Untitled project'}</strong><small>{project.session_id.slice(0, 8)}{project.updated_at ? ` · ${new Date(project.updated_at).toLocaleDateString()}` : ''}</small></span>{project.session_id === sessionId ? <span className="project-current">Current</span> : <ArrowUpRight size={14} />}</button>) : <p className="empty">No projects match this search.</p>}</div>
...
function MobileNavigation({ mainView, onView, onProjects, catalog, agents, status, theme, onClose, onCreateSession, onOpenCapabilities, onToggleTheme, onOpenConnection }: { mainView: MainView; onView: (view: MainView) => void; onProjects: () => void; catalog: CapabilityCatalog; agents: AgentState[]; status: ConnectionStatus; theme: Theme; onClose: () => void; onCreateSession: () => Promise<void>; onOpenCapabilities: (kind: CapabilityKind) => void; onToggleTheme: () => void; onOpenConnection: () => void }) {
  return <div className="mobile-nav-backdrop" onClick={onClose}><aside className="mobile-nav" onClick={(event) => event.stopPropagation()}><div className="brand"><span className="brand-mark"><Sparkles size={16} strokeWidth={2} /></span><span>AgentEvolver</span><button className="mobile-close" onClick={onClose} aria-label="Close navigation">×</button></div><button className="new-chat" disabled={status !== 'connected'} onClick={() => { void onCreateSession(); onClose(); }}><Plus size={16} /> New project</button><nav className="sidebar-section capability-nav view-nav" aria-label="Workspace views">{(['overview', 'chat', 'canvas', 'code', 'science'] as MainView[]).map(view => <button className={mainView === view ? 'view-active' : ''} key={view} onClick={() => { onView(view); onClose(); }}><strong>{humanize(view)}</strong></button>)}</nav><button className="mobile-project-picker" onClick={() => { onProjects(); onClose(); }}><Search size={14} /> Find a project</button><nav className="sidebar-section capability-nav"><p className="eyebrow">Capability library</p>{CAPABILITY_KINDS.map((kind) => <button key={kind} onClick={() => { onOpenCapabilities(kind); onClose(); }}><span><CapIcon kind={kind} /></span><strong>{CAPABILITY_META[kind].label}</strong><em>{catalog[kind].length}</em></button>)}</nav><div className="sidebar-section agents-section"><p className="eyebrow">Active agents</p>{agents.length ? agents.map((agent) => <div className="agent-row" key={agent.name}><span className={`agent-state ${agent.status}`} /><span>{agent.name}</span></div>) : <p className="empty">Agents appear while a task runs.</p>}</div><div className="sidebar-footer"><button className="text-button" onClick={onToggleTheme}>{theme === 'dark' ? <><Sun size={14} /> Light theme</> : <><Moon size={14} /> Dark theme</>}</button><button className="text-button" onClick={() => { onOpenConnection(); onClose(); }}><Settings size={14} /> Connection</button></div></aside></div>;
}
...
function CapabilityDialog({ activeKind, catalog, selection, items, search, onSearch, onSelectKind, onToggle, onToggleAll, onInspect, onClose }: { activeKind: CapabilityKind; catalog: CapabilityCatalog; selection: CapabilitySelection; items: CapabilityItem[]; search: string; onSearch: (value: string) => void; onSelectKind: (kind: CapabilityKind) => void; onToggle: (kind: CapabilityKind, name: string) => void; onToggleAll: (kind: CapabilityKind) => void; onInspect: (kind: CapabilityKind, name: string) => void; onClose: () => void }) {
  ...
  return <div className="modal-backdrop capability-backdrop" onClick={onClose}><section className="capability-dialog" onClick={(event) => event.stopPropagation()}><aside className="capability-menu"><div className="modal-title"><h2>Capabilities</h2></div><p className="eyebrow">Browse capabilities</p>{CAPABILITY_KINDS.map((kind) => <button role="tab" aria-selected={kind === activeKind} className={kind === activeKind ? 'active' : ''} key={kind} onClick={() => onSelectKind(kind)}><span><CapIcon kind={kind} /></span>{CAPABILITY_META[kind].label}<em>{catalog[kind].length}</em></button>)}</aside><section className="capability-content" role="tabpanel"><header><div><p className="eyebrow">Capabilities · {meta.label}</p><h2><CapIcon kind={activeKind} size={20} /> {meta.label}</h2><p>{meta.description}</p></div><button className="close-dialog" onClick={onClose}>×</button></header><div className="capability-toolbar"><input autoFocus value={search} onChange={(event) => onSearch(event.target.value)} placeholder={`Search ${meta.label.toLowerCase()}…`} /><button className="select-all" onClick={() => void onToggleAll(activeKind)}>{allEnabled ? 'Disable all' : 'Enable all'}</button></div><div className="capability-count">{selection[activeKind].length} of {catalog[activeKind].length} enabled for this session</div><div className="capability-list">{items.map((item) => <div className="capability-item" key={item.name}><button className="capability-detail-button" onClick={() => onInspect(activeKind, item.name)}><span className="capability-item-head"><strong>{humanize(item.name)}</strong><span className={`cap-tag cap-${item.source}`}>{item.source === 'extension' ? 'Extension' : 'Default'}</span>{item.evolving ? <span className="cap-tag cap-evolving">Evolvable</span> : null}</span><small>{capabilityDescription(activeKind, item.name)}</small><em>Open {CAPABILITY_META[activeKind].label.slice(0, -1)} details →</em></button><button className={`toggle ${selection[activeKind].includes(item.name) ? 'enabled' : ''}`} onClick={() => void onToggle(activeKind, item.name)} aria-pressed={selection[activeKind].includes(item.name)} aria-label={`Toggle ${item.name}`}><span /></button></div>)}{!items.length ? <p className="empty">No {meta.label.toLowerCase()} match this search.</p> : null}</div></section></section></div>;
}
