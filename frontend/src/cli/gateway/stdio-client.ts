...
export class StdioGatewayClient implements GatewayClient {
  ...
  public async start(): Promise<void> {
    ...
    const python = process.env.AGENTEVOLVER_PYTHON ?? 'python';
    const args = ['-m', 'agentevolver.gateway', '--transport', 'stdio'];
    ...
    this.child.on('exit', (code) => {
      ...
      this.failPending(new Error(`AgentEvolver Gateway exited (${code ?? 'unknown'})`));
