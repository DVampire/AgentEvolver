 ...
 * vice versa. See agentevolver/conversation/README.md.
 *
...
export function ScienceConversation({ request, subscribe, sessionId, connected }: {
  ...
  useEffect(() => subscribe((event) => {
    ...
    if (event.type === 'task.completed' || event.type === 'task.failed') setBusy(false);
  }), [subscribe, sessionId]);
      ...
      <div className="science-thread">
        {entries.length ? entries.map((entry) => (
          ...
          <div className="science-thread-empty">
            ...
            <p>Describe an experiment and the agent runs it, writing its results into
               this project's workspace. The Notebook tab is your own kernel on those
               same files; the Compute tab shows what it is running on.</p>
          </div>
        )}
        {busy ? <div className="science-working"><Loader2 className="science-spinner" /> Working…</div> : null}
        <div ref={bottom} />
      </div>
