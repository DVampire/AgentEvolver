...
React/Vite browser interface for the Python AgentEvolver Gateway. The project overview brings together the living plan, task activity, eight evolvable entity types, staged candidates and workspace files. Chat provides a task composer, live activity timeline, task cancellation, event inspector, automatic reconnect and event replay. A searchable project picker keeps the five workspace views within reach.

The overview reads existing project documents through `project.overview` and relative Markdown plan links through `project.document.read`. Both are bounded, read-only endpoints. Memory shows shared session notes; private actor notes remain separate. Candidate staging and structural validation are displayed separately from any claim of functional improvement.
...
The **Code** view still requires Docker when the gateway runs locally. Prepare
its base image once from the repository root with
`docker build -f docker/base/Dockerfile -t agentevolver/base:latest .`.
The editor image then builds on first use. See
[`docker/vscode/README.md`](../docker/vscode/README.md) for prerequisites and mounts.

The **Science** view uses JupyterLab and ipykernel from the gateway's Python
environment. For local conda setups, install them with
`python -m pip install -e '.[science]'` from the repository root. A missing or
failed kernel is reported in the Science panel; it must start successfully
before the notebook can execute code.
