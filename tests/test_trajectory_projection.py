...
def test_export_prefers_trace_over_a_divergent_live_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agentevolver.trace.server.trace_manager.events",
        lambda _session: _facts(),
    )
...
def test_a_future_reward_label_is_refused_not_replaced_by_an_older_score(
    ...
    with open(path, "a", encoding="utf-8") as handle:
        handle.write(
            '{"schema_version":999,"session_id":"session-1","task_id":"task-1","reward":0.9}\n'
        )
