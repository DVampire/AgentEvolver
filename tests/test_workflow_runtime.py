...
async def test_a_workflow_loaded_as_an_extension_remembers_the_file_it_came_from(tmp_path):
    ...
    try:
        ...
        # The admission cache is machine-level, not a subdirectory of the tree it checks:
        # it used to be `{base_dir}/.checked/`, which put hashed directories inside the
        # tracked component library on every run.
        assert admitted.is_relative_to(path_manager.get(P.ADMISSION))
        assert not admitted.is_relative_to(tmp_path / "extensions")
