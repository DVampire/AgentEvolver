...
async def test_agent_checkpoint_keeps_fixed_contract_and_recent_call_pair(monkeypatch):
    ...
    assert "Preserve the public parser API." not in prompt  # Already retained in fixed context.
...
async def test_failed_scheduled_fold_waits_before_retry_and_resets_after_success(monkeypatch):
    ...
    agent = Agent(retain_recent_steps=4, compact_input_tokens=100_000,
    ...
    async def measure():
        return {"estimated_tokens_after": 100_001 if agent.conversation.checkpoint is None else 50_000,
                "pressure_ratio_after": 0.1}
    agent._measure_compaction_context = measure
...
async def test_provider_overflow_can_recover_during_scheduled_retry_wait(monkeypatch):
    ...
    agent = Agent(retain_recent_steps=1)
    agent.conversation.extend([AssistantMessage(content="old evidence " * 1000),
                               HumanMessage(content="Continue"), AssistantMessage(content="recent")])
    agent.step = 2
