...
async def test_stale_report_read_cannot_acknowledge_r2(feedback_round):
    ...
    assert deployment_manager.acceptance_state(ctx.extra["deployment_contract"], 2, "p03") == "pending"
    builder = WebsiteBuilderAgent()
    builder.ctx = ctx
    state = deployment_manager.feedback_context(ctx)
    rows = json.loads(state.splitlines()[-2])["subscribers"]
    assert next(r for r in rows if r["job_id"] == "p03")["feedback"] == "unread"


@pytest.mark.asyncio
async def test_feedback_and_shared_plan_reach_the_next_builder_request(feedback_round, monkeypatch):
    ...
    async def environment(self, ctx):
        return (await JobEnvironment().get_state(ctx=ctx))["state"]

    monkeypatch.setattr(Agent, "environment_state", environment)
    ...
    assert not ctx.extra["deployment_contract"]["collected_turns"]

    await JobEnvironment().output("p01", turn=2, ctx=ctx)
    assert "feedback_plan_baselines" not in ctx.extra["deployment_contract"]
