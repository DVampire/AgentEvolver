...
The editor image is built automatically on first use by
`VscodeSandbox._ensure_image()`. Its prerequisite, `agentevolver/base:latest`,
must already exist on the gateway's Docker host. This also applies when the
gateway itself runs directly in a host conda environment: the IDE still uses
a container. These AgentEvolver images are built locally; a Docker Hub login
does not provision them.

From the repository root, prepare the base once, then either open Code or build
...
The base includes the full scientific environment and can take substantially
longer than the editor build. Startup reports a missing base or failed editor
build directly, without attempting to pull a nonexistent IDE image afterward.
