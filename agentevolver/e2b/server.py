...
back the same :class:`~agentevolver.sandbox.types.Sandbox` contract with the E2B
code-interpreter SDK (``e2b_code_interpreter``), and register its handles with
the ``E2B`` registry.

TODO: implement E2BSandbox(Sandbox) in agentevolver/e2b/default/ and wire acquire().
"""
...
class E2BManagerServer(BaseModel):
    ...
    async def acquire(self, *args: Any, **kwargs: Any):  # pragma: no cover - scaffold
        raise NotImplementedError(
            "agentevolver.e2b is a scaffold. Implement E2BSandbox in agentevolver/e2b/default/ first. "
