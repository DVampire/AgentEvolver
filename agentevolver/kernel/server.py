...
from agentevolver.kernel.types import Execution, KernelOutput, KernelResult, KernelStatus
    ...
    async def _ensure_server(self, key: str, workspace: Optional[str] = None) -> _Project:
        ...
        async with lock:
            ...
            if importlib.util.find_spec("jupyterlab") is None:
                raise RuntimeError(
                    "Science requires JupyterLab in the gateway's Python environment. "
                    "Install AgentEvolver with the science extra: pip install -e '.[science]'"
                )
...
def _message(msg_type: str, content: dict, session: str) -> dict:
    ...
    return {
        "header": {"msg_id": uuid.uuid4().hex, "username": "agentevolver", "session": session,
