        ...
        return {
            ...
            "properties": {
                "task": {
                    ...
                    "description": (
                        "Self-contained assignment and acceptance contract; put long "
                        "specifications in files. No parent history unless fork=true."
                    ),
                },
                "files": {
                    ...
                    "description": "Existing input/specification paths; omit if none.",
                },
                "read_set": {
                    "type": "array", "maxItems": MAX_DELEGATION_CONTRACT_ITEMS,
                    "items": {"type": "string", "minLength": 1,
                              "maxLength": MAX_DELEGATION_CONTRACT_ITEM_CHARS},
                    "description": "Expected read paths; advisory, not filesystem isolation.",
                },
                "write_set": {
                    "type": "array", "maxItems": MAX_DELEGATION_CONTRACT_ITEMS,
                    "items": {"type": "string", "minLength": 1,
                              "maxLength": MAX_DELEGATION_CONTRACT_ITEM_CHARS},
                    "description": "Expected write paths; parent/child overlaps conflict. Advisory, not isolation.",
                },
                "acceptance": {
                    "type": "array", "maxItems": MAX_DELEGATION_CONTRACT_ITEMS,
                    "items": {"type": "string", "minLength": 1,
                              "maxLength": MAX_DELEGATION_CONTRACT_ITEM_CHARS},
                    "description": "Concrete, independently checkable success conditions.",
                },
                "owner": {"type": "string", "description": "Responsibility label, not a permission grant."},
                "model": {"type": "string", "description": "Registered model route for this invocation."},
                "reasoning_effort": {"type": "string", "enum": ["none", "minimal", "low", "medium", "high", "xhigh"], "description": "Provider reasoning effort for this invocation."},
                "token_budget": {"type": "integer", "minimum": 1, "description": "Cumulative input + output limit per assignment; enforced at reported usage boundaries."},
                "run_in_background": {"type": "boolean", "description": "Return process id now; collect with job__output, stop with job__kill. Blocking dispatches are sequential; background independent work. Files remain shared."},
                "isolate_worktree": {"type": "boolean", "description": "One-shot private Git worktree, returning an archived patch without auto-merge. Not an OS sandbox."},
                "continuable": {"type": "boolean", "description": "Background only: retain conversation for follow-ups via send_message_tool. Default false (one-shot)."},
                "subscription_topics": {"type": "array", "items": {"type": "string"}, "description": "Serialized event turns; requires background + continuable. Registration is idle; the standing task runs only on events."},
                "fork": {"type": "boolean", "description": "Include parent conversation in child context; useful when prior findings/decisions are essential. Default false for a fresh, self-contained task."},
                "target_name": {"type": "string", "description": "Capability being generated, optimized or evaluated; only for those roles."},
                "target_type": {"type": "string", "enum": ["tool", "skill", "agent", "connector", "memory", "plugin", "workflow", "environment"], "description": "Component kind for capability changes, required also for new targets so installation uses the right manager."},
                "tool_allowlist": {"type": "array", "items": {"type": "string"}, "description": "Evolution probe: exact allowed tools; [] means none."},
                "skill_allowlist": {"type": "array", "items": {"type": "string"}, "description": "Evolution probe: exact allowed skills; [] means none."},
                "connector_allowlist": {"type": "array", "items": {"type": "string"}, "description": "Evolution probe: exact allowed connectors; [] means none."},
                "plugin_allowlist": {"type": "array", "items": {"type": "string"}, "description": "Exact allowed plugins; [] means none."},
                "environment_allowlist": {"type": "array", "items": {"type": "string"}, "description": "Exact allowed environments; [] means none."},
                "workflow_allowlist": {"type": "array", "items": {"type": "string"}, "description": "Evolution probe: exact allowed workflows; [] means none."},
            },
            "required": ["task"],
