"""AgentContextManager: agent registration, lifecycle and version history.

The module convention every manager follows — ``context.py`` holds the manager,
``server.py`` the public facade, exactly as in ``model/`` and ``tool/``.

What used to share this file was model-facing context: the four-layer envelope, the
prompt assembler, the project instruction loader. That is a different subject with a
different lifetime, and it now lives in :mod:`agentevolver.agent.context`.
"""
...
from agentevolver.agent.types import AgentConfig, AgentContext

if TYPE_CHECKING:  # `Agent` is only ever a type here, and importing it eagerly would
    # close a cycle: types imports the loop, the loop imports this package's assembler,
    # and importing the package runs this module.
...
class AgentContextManager(BaseModel):
    ...
    async def initialize(self, agent_names: Optional[List[str]] = None) -> None:
        ...
        # Imported here, not at module scope: `agent.types` imports the loop, the loop
        # imports this package's assembler, and importing the package runs this module.
        # By the time anything initializes, that cycle has already resolved.
        from agentevolver.agent.types import Agent
