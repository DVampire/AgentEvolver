"""ConnectorOptimizeAgent — evolves an existing connector's CONNECTOR.md given an evolution task."""
...
class ConnectorOptimizeAgent(Agent):
    ...
    ``__call__``. The target connector is named in the task; the agent should call ``inspect_capability_tool`` first
    ...
    name: str = Field(default="connector_optimize_agent")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            prompt_name=prompt_name or "connector_optimize_agent",
    ...
    async def _finalize_run(self, response, ctx):
        """Run the base loop, then reload and re-register the edited connector."""
        ...
        if response.success:
            ...
            if result.decision == HookDecision.BLOCK:
                ...
                response.message = result.reason or "Re-registration failed; include the edited CONNECTOR.md path in the done_tool reasoning."
        return response
