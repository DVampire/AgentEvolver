...
Checks, in order of how badly each one breaks the suite:
  ...
  3. Every module and reference is present and reachable from the entry file.
     Modules live under `references/modules/` because this framework hands an agent
     the files it finds in `scripts/`, `references/`, `resources/` and `examples/`
     and nowhere else — a top-level `modules/` was invisible, so the entry file's
     routing table pointed at paths the agent could not open.
...
#: `name` and `description` are what a skill cannot load without — the registry key and
#: the line an agent decides from. The rest are this framework's: `SkillConfig` reads them
#: and gives each a default, so a skill omitting them still loads, but every other skill in
#: the tree declares them and a reviewer comparing two of them should not have to work out
#: which fields are missing versus which are defaulted.
REQUIRED_FRONTMATTER_KEYS = {"name", "description"}
FRONTMATTER_KEYS = REQUIRED_FRONTMATTER_KEYS | {
    "version", "type", "license", "category", "requirements", "enable_evolving",
...
def main():
    ...
    if entry_text.startswith("---\n"):
        ...
        for required in sorted(REQUIRED_FRONTMATTER_KEYS):
    ...
    for name in MODULES:
        rel = os.path.join("references", "modules", name + ".md")
