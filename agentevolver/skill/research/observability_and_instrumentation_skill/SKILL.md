---
name: observability_and_instrumentation_skill
...
**NOT for:**
- Diagnosing a failure happening right now — use the `debugging_and_error_recovery_skill` skill (observability is what makes that skill fast next time)
- Profiling and optimizing measured slowness — use the `performance_optimization_skill` skill
- Launch-day monitoring checklists and rollback triggers — see the `shipping_and_launch_skill` skill; this skill covers the instrumentation that feeds them
...
**Never log secrets, tokens, passwords, or full PII.** This is a hard rule from the `security_review_skill` skill — telemetry pipelines are a classic data-leak path. Allowlist fields; don't log whole request bodies.
