...
> **Interactivity note.** This skill assumes a responsive user. In autonomous / non-interactive runs (batch, `/loop`, scheduled), do not block on questions — surface assumptions and proceed, or defer the interactive step to the orchestrator / user turn.
...
This skill closes the gap before it costs anything. The other Define-phase skills assume you already know roughly what you want: `idea_refine_skill` generates variations from an idea, `spec_driven_development_skill` writes the requirements down, `doubt_driven_development_skill` stress-tests a plan after you've drafted one. Interview-me is the part before all of those, where you ask one question at a time, with your best guess attached, until you can predict what the user is going to say before they say it.
...
- **`idea_refine_skill`**: downstream. If the confirmed intent is "I want X but I don't know how to scope it," hand off to `idea_refine_skill` to generate variations against the now-explicit intent.
- **`spec_driven_development_skill`**: downstream. If the confirmed intent is concrete ("I want X for Y users with Z success criteria"), hand off to `spec_driven_development_skill` to write it down.
- **`planning_and_task_breakdown_skill`**: two hops downstream of this skill (after the spec).
- **`doubt_driven_development_skill`**: opposite end of the timeline. Interview-me is pre-decision intent extraction; doubt-driven is post-decision artifact review. Both catch divergence, but at different moments.
- **`source_driven_development_skill`**: orthogonal. Interview-me clarifies what the user wants; SDD verifies framework facts. They don't compete.
...
After applying interview_me_skill:
...
- [ ] Any handoff to a downstream skill (`idea_refine_skill`, `spec_driven_development_skill`) was framed in terms of the confirmed intent, not the original underspecified ask
