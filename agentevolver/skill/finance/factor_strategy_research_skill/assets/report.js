...
async function main(){
  ...
  if(data.test_history)basis.append(node("p",data.test_history.interpretation,"warning"));
  if(data.record){const r=data.record;basis.append(node("p",`Round: ${r.round_id} · Report: ${r.report_id} · Snapshot phase: ${r.phase}`));if(r.parent_report_id)basis.append(node("p",`Previous report: ${r.parent_report_id}`));}
  for(const kind of ["factors","strategies"]){const section=document.getElementById(kind), inventory=section.querySelector(".inventory");
    if(kind==="strategies" && data.strategy_counts){const n=data.strategy_counts;inventory.append(node("p",`${n.candidate_hypotheses} formal hypotheses (${n.evaluated_candidate_hypotheses} evaluated) · ${n.candidate} candidate versions · ${n.ablation} ablations · ${n.benchmark} benchmarks · ${n.legacy_unclassified} unclassified historical records`));}
    ...
    data[kind].forEach(c=>{const article=node("article",null,"candidate");article.id=`candidate-${c.id}`;article.append(node("h3",`${c.id} · ${c.name}`),node("p",c.status,"status"),node("p",c.definition,kind==="factors"?"formula":"rules"));
      ...
      if(c.strategy_spec)article.append(table(["Parameter","Executed value"],Object.entries(c.strategy_spec.parameters).map(([key,value])=>[key,JSON.stringify(value)])));
      if(c.parent_ids?.length)article.append(candidateLinks("Parents: ",c.parent_ids));
      if(c.research_role)article.append(node("p",`Research role: ${c.research_role.replaceAll("_"," ")}`));
      if(c.control_for?.length)article.append(candidateLinks("Control for: ",c.control_for));
