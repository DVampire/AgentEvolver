"""Vectara RAG — from the Langflow `vectara` vector-store bundle (ported)."""
...
class VectaraVectaraRagPlugin(VectorStorePlugin):
    name: str = "vectara.vectara_rag"
    display_name: str = 'Vectara RAG'
    description: str = 'Vectara'
    ...
    async def __call__(self, vectara_customer_id: str = "", vectara_corpus_id: str = "", vectara_api_key: str = "", query: str = "", texts: Optional[List[str]] = None,
        ...
        return await self._run(query=query, texts=texts, embedding=embedding, k=int(k),
            vectara_customer_id=vectara_customer_id, vectara_corpus_id=vectara_corpus_id, vectara_api_key=vectara_api_key)
