...
tools: 10
implemented: 10
credentials: [ASTRA_DB_API_ENDPOINT, ASTRA_DB_APPLICATION_TOKEN, ASTRA_DB_ID, HCD_API_ENDPOINT, HCD_TOKEN, OPENAI_API_KEY]
...
| `datastax.astradb_data_api` | Astra DB Data API | ✅ |  |
...
- `datastax.astradb_cql` → `data` keys: count, records
- `datastax.astradb_data_api` → `data` keys: count, records
- `datastax.dotenv` → `data` keys: count, variables
