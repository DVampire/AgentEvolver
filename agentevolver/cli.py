...
The ``agentevolver`` command owns all user-facing modes: a one-shot control
...
class GatewayLauncher:
    ...
    def from_args(cls, argv: Optional[Sequence[str]]) -> "GatewayLauncher":
        parser = argparse.ArgumentParser(prog="agentevolver serve", description="Run the AgentEvolver Gateway")
...
class Console:
    """The ``agentevolver`` entry: routes serve / tui / <command> to the Gateway."""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    """Console entry point (pyproject: ``agentevolver = agentevolver.cli:main``)."""
    return Console(sys.argv[1:] if argv is None else argv).run()


def gateway_main(argv: Optional[Sequence[str]] = None) -> int:
    """Start the Gateway directly — used by ``python -m agentevolver.gateway`` and tests."""
