...
_IMAGE = "agentevolver/vscode:latest"
_BASE_IMAGE = "agentevolver/base:latest"
# repo root: .../agentevolver/sandbox/default/vscode.py -> parents[3]
...
class VscodeSandbox(OpenSandbox):
    ...
    async def _ensure_image(self) -> None:
        ...
        if not (_DOCKERFILE_DIR / "Dockerfile").exists():
            raise RuntimeError(
                f"The IDE image {image} is missing, and its build files were not found at "
                f"{_DOCKERFILE_DIR}. Build the image from an AgentEvolver source checkout "
                "on the gateway's Docker host, then retry Code."
            )
        if await self._docker(["image", "inspect", _BASE_IMAGE], quiet=True) != 0:
            # This is a locally built prerequisite, not a published Docker Hub
            # image. Stop here instead of letting an unsuccessful IDE build
            # fall through to a misleading registry authentication error.
            raise RuntimeError(
                f"The IDE needs the local base image {_BASE_IMAGE}. "
                "On the gateway's Docker host, run this from the AgentEvolver repo root: "
                "docker build -f docker/base/Dockerfile -t agentevolver/base:latest . "
                "Then retry Code; the editor image will build automatically."
            )
        ...
        if code != 0:
            raise RuntimeError(
                f"Building the IDE image {image} failed (Docker exit {code}). "
                "Check the Docker build output in the gateway log, fix the build error, "
                "then retry Code."
            )
