...
def forget(sandbox_id: str) -> None:
    ...
    if not sandbox_id:
        return
    _write([item for item in _load() if item != str(sandbox_id)])
...
async def _remove_container(name: str) -> bool:
...
async def reap_stale() -> List[str]:
    """Force-remove containers recorded by a previous (dead) run.

    Containers are named ``sandbox-<id>`` by the opensandbox daemon. A missing
    container is not an error — the goal is simply that nothing from the
    ledger can squat on resources for the new run. Entries stay in the ledger
    when removal is impossible (e.g. no socket and no CLI) so a later boot can
    retry.
    """
    ids = _load()
    if not ids:
    ...
    for sandbox_id in ids:
        ...
        try:
            if await _remove_container(name):
                reaped.append(name)
            else:
...
__all__ = ["forget", "reap_stale", "record", "stale_ids"]
