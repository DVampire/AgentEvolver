...
class AgentGateway:
    ...
    async def _command_project_overview(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Read the selected project's plan and shared notes without starting work."""
        from agentevolver.gateway.overview import project_documents

        session_id = self._require_session_id(params)
        session = self._sessions[session_id]
        documents = await asyncio.to_thread(project_documents, session.sandbox)
        active_tasks = sum(
            not task.done() for task_id in getattr(session, "task_ids", [])
            if (task := self._active_agent_tasks.get(task_id)) is not None
        )
        return {"session_id": session_id, **documents,
                "runtime": {"active_tasks": active_tasks},
                "plan_mode": plan_manager.state(session.context.id).summary()}

    async def _command_project_document_read(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Follow a plan's relative document link inside this project's plan root."""
        from agentevolver.gateway.overview import read_document

        session = self._sessions[self._require_session_id(params)]
        relative = str(params.get("path") or "")
        path = Path(relative)
        if not relative or path.is_absolute() or ".." in path.parts or path.suffix.lower() != ".md":
            raise ValueError("Choose a relative Markdown document inside the project plan")
        return await asyncio.to_thread(read_document, session.sandbox.plan_root, relative)
    ...
    async def _command_science_start(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await kernel_manager.execute("", key=session_id, workspace=str(workspace), origin="user")
        if not result.success:
            raise RuntimeError(result.error or "The project kernel could not start")
