...
class AgentGateway:
    ...
    # ------------------------------------------------------------------
    # Docs — the repository's own documentation, served to the reader
    # ------------------------------------------------------------------
    #: What the docs browser may open, in reading order. Each entry is a section
    #: title and either explicit repo-relative paths or one glob.
    #:
    #: This is an allowlist, not a starting directory. `docs.get` serves a path only
    #: when this listing already offered it, so the pair is a document browser rather
    #: than a repository file reader — a distinction that matters because the gateway
    #: answers whoever reached its socket, and "under the repo root" includes every
    #: source file, every config, and `.env`.
    _DOC_SECTIONS: tuple = (
    ...
    def _repository_root() -> Path:
        """The checkout this gateway runs from — `agentevolver/gateway/service.py` → up 3."""
        return Path(__file__).resolve().parents[2]

    @staticmethod
    def _document_title(path: Path, relative: str) -> str:
        """A document's own first heading, or its filename when it has none.

        Read from the file rather than derived from the path: `docs/decisions/` names
        are dated slugs, and a sidebar of `2026-08-15-coverage-gate-is-a-dark-file-register`
        is unreadable next to the title the author already wrote.
        """
        try:
    ...
    def _doc_entries(self) -> list:
        """Every document the browser may open, deduplicated, in section order."""
        root = self._repository_root()
        entries: list = []
        seen: set = set()
        for section, explicit, pattern in self._DOC_SECTIONS:
            paths = [root / name for name in explicit]
            if pattern:
                # A section's `README.md` is its index and leads, whatever it sorts as:
                # `docs/decisions/` holds dated slugs, so plain alphabetical order buries
                # the page that explains what the other files in the section are.
                paths.extend(sorted(root.glob(pattern),
                                    key=lambda item: (item.name != "README.md", item.name)))
            for path in paths:
                try:
                    relative = str(path.resolve().relative_to(root))
                except ValueError:
                    continue
                # A glob can re-offer a file an earlier section named explicitly
                # (`docs/*.md` catches `docs/DOC-STANDARD.md`). First section wins,
                # so the curated ordering above survives.
                ...
                entries.append({
                    "path": relative,
                    "section": section,
                    "title": self._document_title(path, relative),
                })
        return entries

    async def _command_docs_list(self, _: Dict[str, Any]) -> Dict[str, Any]:
        """The document index: what the reader can open, grouped and titled."""
        return {"documents": self._doc_entries()}

    async def _command_docs_get(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """One document's Markdown, by the path `docs.list` gave out.

        Refusing an unlisted path is the whole security property here. Containment
        under the repository root is not enough on its own: it admits every source
        file and every config in the checkout.
        """
        requested = str(params.get("path") or "")
        entry = next((item for item in self._doc_entries() if item["path"] == requested), None)
        if entry is None:
            raise ValueError(f"Unknown document: {requested}")
        root = self._repository_root()
        content, relative = self._read_repository_file(root, root / requested)
        if relative is None:
            raise ValueError(f"Document is no longer readable: {requested}")
