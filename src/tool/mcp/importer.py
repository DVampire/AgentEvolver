...
registers them into the tool manager (`src.tool.server.tool_manager`) so agents can call them like native tools.
"""
...
- Registers one MCP proxy tool per server into tool manager
...
- name_prefix (str): Prefix for registered tool manager tool names (default: "mcp").
- override (bool): Whether to override existing tool manager tool registrations.
