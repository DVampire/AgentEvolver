"""Skill Generator Tool - A workflow tool for creating skills and registering them to skill manager.
...
_SKILL_GENERATOR_DESCRIPTION = """Skill generator tool that creates complete skill packages and registers them in skill manager.
This tool will:
...
5. Write files to disk, register in skill manager, and run test cases
...
class SkillGeneratorTool(Tool):
    """A workflow tool that generates complete skill packages and registers them in skill manager."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if base_dir is not None:
            ...
            self.base_dir = assemble_project_path("workdir/skill_generator_tool")
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        try:
            ...
            # Step 6 — Register in skill manager
            logger.info("📝 Step 6: Registering skill in skill manager...")
            try:
                skill_config = await self._register_skill(skill_dir)
                now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                plan.log.append(f"[{now}] Registered in skill manager: {skill_config.name} v{skill_config.version}")
                self._write_plan(plan, skill_dir)
    ...
    async def _register_skill(self, skill_dir: str):
        """Register the skill directory in skill manager."""
    ...
    async def _test_skill(self, skill_name: str, skill_dir: str) -> Tuple[bool, str]:
        """Load test cases from disk and run them via skill manager in parallel.
