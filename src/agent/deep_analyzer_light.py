...
class DeepAnalyzerLightAgent(Agent):
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        logger.info(f"| 🔍 DeepAnalyzerLightAgent Task: {task}, Attachments: {files}.")

        try:
            analysis_prompt = (
                f"Task: {task}\n\n"
                f"Please provide a clear, well-reasoned response."
            )
            images = [f for f in (files or []) if self._is_image(f)]

            # Run all analyzer models in parallel
            results = await asyncio.gather(
                *[self._analyze(model, analysis_prompt, images) for model in self.analyzer_llm_models],
            ...
            if len(analyses) == 1:
                summary = analyses[0]["analysis"]
            else:
                summary = await self._summarize(analysis_prompt, analyses)
    ...
    def _build_messages(self, analysis_prompt: str, images: List[str]) -> list:
        ...
        if not images:
            return [system, HumanMessage(content=analysis_prompt)]

        content = [ContentPartText(text=analysis_prompt)]
        for img in images:
    ...
    async def _analyze(self, model: str, analysis_prompt: str, images: List[str]) -> Optional[str]:
        messages = self._build_messages(analysis_prompt, images)
        resp = await model_manager(model=model, messages=messages)
        text = resp.message.strip() if resp else ""
        return text or None

    async def _summarize(self, analysis_prompt: str, analyses: List[Dict[str, str]]) -> str:
        from src.message.types import HumanMessage, SystemMessage
