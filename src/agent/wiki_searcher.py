"""Wiki Searcher Agent — a tool-calling agent restricted to wiki_search_skill."""
...
# Tools allowed for WikiSearcherAgent (besides wiki_search_skill dispatched as skill)
_ALLOWED_TOOLS = {"bash_tool", "todo_tool", "done_tool"}


@AGENT.register_module(force=True)
class WikiSearcherAgent(Agent):
    """Agent that answers questions by searching Wikipedia via wiki_search_skill.

    Only the wiki_search_skill is available as a skill action, and only
    bash_tool / todo_tool / done_tool are available as tool actions.
    """

    model_config = ConfigDict(arbitrary_types_allowed=True, extra="allow")

    name: str = Field(default="wiki_searcher_agent", description="The name of the wiki searcher agent.")
    description: str = Field(
        default="A Wikipedia research agent that uses wiki_search_skill to find and synthesize encyclopedic information.",
        description="The description of the wiki searcher agent.",
    )
    metadata: Dict[str, Any] = Field(default={}, description="The metadata of the wiki searcher agent.")
    ...
    async def _get_tool_context(self, ctx: SessionContext, record: Record = None, **kwargs) -> Dict[str, Any]:
        """Get tool context — only exposes the allowed tools."""
    ...
    async def _get_skill_context(self, ctx: SessionContext, **kwargs) -> Dict[str, Any]:
        """Get skill context — only wiki_search_skill."""
        skill_content = await skill_manager.get_context(skill_names=["wiki_search_skill"])
        if not skill_content:
            skill_context = "<skill_context>[wiki_search_skill is not loaded.]</skill_context>\n"
        else:
        ...
        try:
            ...
            for i, action in enumerate(actions):
                ...
                if action_type == "skill":
                    # Only wiki_search_skill is permitted
                    if action_name != "wiki_search_skill":
                        logger.warning(f"| ⚠️  Skill '{action_name}' is not allowed in WikiSearcherAgent, skipping.")
                        action_results.append({**action.model_dump(), "output": f"Skill '{action_name}' is not available in this agent."})
                        continue
                    ...
                    # Tool action — only allowed tools
                    if action_name not in _ALLOWED_TOOLS:
                        logger.warning(f"| ⚠️  Tool '{action_name}' is not allowed in WikiSearcherAgent, skipping.")
                        action_results.append({**action.model_dump(), "output": f"Tool '{action_name}' is not available in this agent."})
                        continue
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        """Main entry point for the wiki searcher agent.

        Args:
            task: The question or research task to answer using Wikipedia.
            files: Optional attached files.

        Returns:
            AgentResponse with the Wikipedia-sourced answer.
        """
        logger.info(f"| 🚀 Starting WikiSearcherAgent: {task}")
        ...
        logger.info(f"| ✅ WikiSearcherAgent completed after {step_number}/{self.max_steps} steps")
