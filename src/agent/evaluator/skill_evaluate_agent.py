...
class SkillEvaluateAgent(Agent):
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        logger.info(f"| 🚀 Starting SkillEvaluateAgent: {task} (skill={target_name})")
        ...
        if not target_name:
            logger.warning("| ⚠️ SkillEvaluateAgent called without target_name")
        ...
        if step_number >= self.max_steps and not response["done"]:
            ...
            response = {
                "done": False,
                "result": "Skill evaluation did not complete within max steps.",
        ...
        logger.info(f"| ✅ SkillEvaluateAgent completed after {step_number}/{self.max_steps} steps")
