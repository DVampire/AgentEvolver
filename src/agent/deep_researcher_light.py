...
class DeepResearcherLightAgent(Agent):
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        logger.info(f"| 🔍 DeepResearcherLightAgent starting (concise original task): {task}")

        try:
            search_results = await self._search(original_task, task, filter_year)
    ...
    async def _search(self, original_task: str, task: str, filter_year: Optional[int]) -> List[Dict[str, Any]]:
        ...
        if self.use_llm_search:
            ...
            prompt = (
            f"Below is the overall task objective for context and guidance.\n\n"
            f"Original Task: {original_task}\n\n"
            f"The Original Task is provided to help you understand the broader objective and guide your research direction. "
            f"You may use it to identify relevant context, constraints, entities, or candidate options worth investigating. "
            f"For example, if the Original Task includes multiple-choice options, you may investigate those options to better support the assigned Task. "
            f"However, your primary goal is not to solve the Original Task itself, but to support the specific assigned task below, so you don't need to give the answer of the Original Task.\n\n"
            f"Your responsibility is to research the following assigned task and provide a comprehensive summary with citations where possible.\n\n"
            f"Task: {task}\n\n"
            f"Focus your research and response primarily on the assigned Task, while using the Original Task only as supporting context when helpful."
            )
            # prompt = (
