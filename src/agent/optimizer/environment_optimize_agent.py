"""EnvironmentOptimizeAgent — evolves an existing environment given an evolution task."""
...
class EnvironmentOptimizeAgent(Agent):
    """Evolves an existing environment (Python class and/or config) to satisfy an evolution task.
    ...
    (require_grad=True) and to obtain its file paths — a frozen environment must NOT be optimized."""

    model_config = ConfigDict(arbitrary_types_allowed=True, extra="allow")

    name: str = Field(default="environment_optimize_agent")
    description: str = Field(
        default="An agent that evolves an existing environment (Python class and/or config) given an evolution task."
    )
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        """Run the base loop, then reload and re-register the edited environment."""
        ...
        if response.success:
            ...
            if result.decision == HookDecision.BLOCK:
                ...
                response.message = result.reason or "Re-registration failed; include the edited environment file path in the done_tool reasoning."
        return response
