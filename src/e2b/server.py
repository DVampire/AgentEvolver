...
back the same :class:`~src.sandbox.types.Sandbox` contract with the E2B
code-interpreter SDK (``e2b_code_interpreter``), and register its handles with
the ``E2B`` registry.

TODO: implement E2BSandbox(Sandbox) in src/e2b/default/ and wire acquire().
"""
...
class E2BManagerServer(BaseModel):
    ...
    async def acquire(self, *args: Any, **kwargs: Any):  # pragma: no cover - scaffold
        raise NotImplementedError(
            "src.e2b is a scaffold. Implement E2BSandbox in src/e2b/default/ first. "
