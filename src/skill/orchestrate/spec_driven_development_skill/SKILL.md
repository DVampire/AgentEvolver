---
name: spec_driven_development_skill
...
> Follow `planning_and_task_breakdown_skill` for the full task-sizing and dependency-ordering mechanics; it is the canonical source. The template below is a lightweight inline form; if they ever diverge, `planning_and_task_breakdown_skill` takes precedence.
...
Execute tasks one at a time following `incremental_implementation_skill` (`incremental_implementation_skill`) and `test_driven_development_skill` (`test_driven_development_skill`). Use `context_engineering_skill` (`context_engineering_skill`) to load the right spec sections and source files at each step rather than flooding the agent with the entire spec.
