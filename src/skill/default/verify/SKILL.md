...
**Check the available skills first — even if you already know how to
build and run.** A skill that already covers running/verifying this app
is the repo's verified path: follow it instead of rediscovering. (The
`run` skill is the general fallback for launching an app.)

- **A skill matches your surface** → use it and follow its setup.
- **No matching skill** → cold start from README/package.json/Makefile.
  Timebox ~15min. Stuck → BLOCKED with exactly where. Got through → note
  the working build/launch recipe.
...
a first-time user would trip on. Lower the bar: if it made you pause,
it goes here. But the pause has to be yours, from running the app —
not from reading the PR page. Claim/diff mismatch, pre-existing
breakage, and env notes also belong.

Each probe gets a line here even when it held — "🔍 empty `--from`
→ clean error" tells the author what *was* covered.

Lead with ⚠️ for lines worth interrupting the reviewer for; plain
bullets are context. Empty is fine if nothing stuck out.>
```
...
Deliver the report by calling `done_tool` (`result` = the full verdict +
steps + findings above). Drive the app with `bash_tool`; to verify a rendered
HTML/web surface, use the `artifact_renderer` env action for a screenshot.
