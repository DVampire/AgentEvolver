...
from src.logger import logger, set_session_id
from src.session import SessionContext
from src.task import Task
...
class AgentBus:
    ...
    async def _run_planner_loop(self, message: BusMessage) -> None:
        ...
        round_results: List[Dict[str, Any]] = []  # results from previous round (parallel to dispatches)
        ...
        tracer = Tracer()
        # Start fresh — avoid merging records from previous runs of the same task
        if os.path.exists(tracer_save_path):
            os.remove(tracer_save_path)
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            if len(sub_messages) == 1:
                # Single agent — just await directly
                raw_responses = [
                    await self._call_agent(
                        sub_messages[0].recipients[0], sub_messages[0], ctx=ctx
                    )
                ]
            else:
                # Multiple agents — track completion of each
                # Use index-based tracking to handle duplicate agent names
                futures = [
                ...
                # Map future id to (index, agent_name, subtask)
                future_info = {
                    id(f): (i, agent_names[i], dispatches[i].get("task", "")[:60])
                    for i, f in enumerate(futures)
                }
                raw_responses = [None] * len(futures)
                completed_count = 0

                for coro in asyncio.as_completed(futures):
                    result = await coro
                    # Find which future this was
                    for f in futures:
                        if f.done() and id(f) in future_info:
                            idx, name, subtask = future_info.pop(id(f))
                            raw_responses[idx] = result
                            completed_count += 1
                            remaining = len(futures) - completed_count
                            elapsed = _time.time() - _dispatch_start
                            set_session_id(session_id)
                            if remaining > 0:
                                pending = [
                                    f"{future_info[id(ff)][1]}({future_info[id(ff)][2]})"
                                    for ff in futures if id(ff) in future_info
                                ]
                                logger.info(
                                    f"| Bus: agent '{name}' completed ({elapsed:.0f}s), "
                                    f"waiting for {remaining} more: {pending}"
                                )
                            else:
                                logger.info(
                                    f"| Bus: agent '{name}' completed ({elapsed:.0f}s), "
                                    f"all {len(futures)} agents done"
                                )
                            break

            set_session_id(session_id)
            total_elapsed = _time.time() - _dispatch_start
            logger.info(f"| Bus: round {round_num} agents finished ({total_elapsed:.0f}s)")
            ...
            for d, raw in zip(dispatches, raw_responses):
                ...
                if isinstance(raw, BaseException):
                    ...
                    round_results.append({"success": False, "error": str(raw)})
