...
Step 2 — Match the task to a skill by reading each skill's description and judging whether the task's domain and problem type fall within its scope. Use the table below as a quick reference; for skills not listed, rely on the `Description` field in `<skill_context>` directly.

| Skill | When to use |
|-------|-------------|
| `markov_hitting_time_skill` | Computing expected steps, first-passage times, escape probabilities, or similar hitting-time quantities on Markov chains |
| `discrete_linear_system_skill` | Analyzing or designing discrete-time state-space systems, observers, controllers, or z-domain problems |
| `differential_equation_integration_skill` | Solving ODEs, PDEs, or integrals, including Laplace-transform and continuous-time system problems |
| `cs_algorithm_complexity_skill` | Reasoning about time/space complexity, algorithm design, computability, or cryptographic problems |
| `abstract_algebra_topology_skill` | Problems in group theory, rings, fields, modules, topology, or representation theory |
| `bio_med_experimental_design_skill` | Determining required controls for biological experiments, or matching a sequence motif to a kinase/enzyme family |
| `protein_engineering_skill` | Designing amino acid substitutions that abolish a biochemical property (charge, phosphorylation, binding) while preserving the structural role of the mutated region (IDR, linker, ordered domain) |
| `cancer_genomics_scoring_skill` | Computing a numeric clonal expansion or tumorigenic score from chromosomal CNV events, per-gene weights, and regulatory modifiers (repressors/activators on specific chromosomes) |
| `pharmacy_counseling_skill` | Identifying the most important counseling recommendation for a patient's medication regimen, including contraindications, drug-disease conflicts, and drug interactions given patient history and prescriptions |
| `pharmacy_compounding_skill` | Determining beyond-use dates (BUD), sterile preparation requirements, and container handling rules based on USP <797>/<800> compounding standards |
| `rnaseq_filtering_skill` | Designing a filtering strategy based on log2 fold change to remove contaminating non-target cell signals from RNA-seq data while preserving true biological differences between conditions |
| `pediatric_clinical_calculation_skill` | Computing pediatric fluid volumes or rates (resuscitation bolus, Holliday-Segar maintenance, deficit replacement) across multi-phase regimens, including burn patients, dehydration, and co-administered fluid adjustments |
| `environmental_risk_assessment_skill` | Calculating soil-porewater partitioning (Koc/Kd), plant uptake via TSCF, estimated daily intake, hazard quotient, BAF/BCF for PFAS, and mixture toxicity (synergism/antagonism) from environmental contamination scenarios |
| `dna_translation_skill` | Translating a DNA sequence to amino acids by identifying the correct reading frame and strand (including reverse complement / 6-frame translation) to find the first meaningful ORF |
| `image_grounded_expert_reasoning_skill` | Questions that require reading and interpreting a provided image, figure, or diagram |
| `materials_property_prediction_skill` | Predicting or identifying materials properties from experimental data (XRD, optical, descriptors) under physical/chemical constraints |
| `organic_synthesis_skill` | Determining the product, IUPAC name, or stereochemistry (R/S, E/Z) of a named organic reaction (Wittig, aza-Claisen, Nef, ring-closing metathesis, etc.), or tracing atoms through a multi-step synthesis |
| `organic_structure_elucidation_skill` | Determining the structure of an unknown organic compound from combustion analysis (CO₂/H₂O masses), molar mass, IR/spectroscopic clues, wet chemical tests (Tollens, FeCl₃, Lucas), and degradation products (ozonolysis, HBr monobrominated derivatives) |
| `novel_spec_simulation_skill` | Simulating systems defined by a novel or non-standard specification provided in the problem |
| `theoretical_physics_modeling_skill` | Modeling problems in quantum field theory, gauge theory, renormalization, or string theory |
| `variational_nonlinear_pde_skill` | Solving variational problems, nonlinear PDEs, or Euler-Lagrange equations |
| `factual_knowledge_lookup_skill` | Factual knowledge lookup across any domain (humanities, chemistry, biology, pop culture) where the answer is a unique proper noun, integer, color name, or classification that must be anchored to a specific authoritative source or catalog — including named molecule atom counts (e.g. "how many fluorines in perfluoronanocar", "how many carbons in mercedesbenzene"), allotrope/isomer counts, and brand/product classifications |
| `diptera_appendage_counter_skill` | Counting total locomotor appendages across a mixed collection of larvae or invertebrates |
