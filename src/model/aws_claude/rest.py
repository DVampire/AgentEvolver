...
class AwsClaudeCompletions:
    """AWS Claude completions API client (OpenAI-compatible /chat/completions)."""
    ...
    def _get_headers(self) -> Dict[str, str]:
        """Get headers for the AWS Claude API request."""
        headers = {
    ...
    def _get_api_url(self) -> str:
        """Get the API URL for the AWS Claude endpoint."""
    ...
    async def create(
        ...
        **kwargs: Any,
        ...
        """
        Create a chat completion (OpenAI-compatible /chat/completions).
        ...
        for key, value in kwargs.items():
            if value is None:
                continue
            if key == "max_completion_tokens":  # gateway uses max_tokens
        ...
        try:
            ...
            logger.error(f"AWS Claude API HTTP error: {e}")
            try:
                error_detail = e.response.json()
                raise Exception(f"AWS Claude API request failed: {error_detail}")
            except Exception:
                raise Exception(f"AWS Claude API request failed: {e.response.text}")
        except httpx.RequestError as e:
            logger.error(f"AWS Claude API request error: {e}")
            raise Exception(f"AWS Claude API request failed: {str(e)}")
        except Exception as e:
            logger.error(f"Unexpected error in AWS Claude API request: {e}")
            raise


class AwsClaudeChatNamespace:
    """Chat namespace for AwsClaudeClient (similar to OpenAI's chat namespace)."""

    def __init__(self, completions: AwsClaudeCompletions):
        self.completions = completions


class AwsClaudeClient:
    """AWS Claude API client (OpenAI-compatible, similar to AsyncOpenAI)."""
    ...
    def __init__(
        ...
        completions = AwsClaudeCompletions(
            api_key=api_key,
            base_url=base_url,
        ...
        # Create chat namespace for compatibility (client.chat.completions.create)
        self.chat = AwsClaudeChatNamespace(completions=completions)
