...
class GeneralMemorySystem(Memory):
    ...
    base_dir: Optional[str] = Field(default=None, description="Base directory for storing memory data")
    ...
    async def start_session(self,
        ...
        if ctx is None:
            ctx = MemoryContext(id=generate_unique_id("session"))
        id = ctx.id
        ...
        if not isinstance(event_type, EventType):
            ...
            if isinstance(event_type, str):
                try:
                    ...
                    logger.warning(f"| ⚠️ Invalid event_type '{event_type}', defaulting to ACTION_STEP")
                    event_type = EventType.ACTION_STEP
            else:
                logger.warning(f"| ⚠️ Invalid event_type type '{type(event_type)}', defaulting to ACTION_STEP")
                event_type = EventType.ACTION_STEP
