...
def parse_args():
    ...
    parser.add_argument(
        "--task",
        default=(
            "Improve the hello_world_tool so that it also returns a timestamp "
            "alongside the greeting message. The timestamp should be in ISO 8601 format "
            "and included both in the response message string and in the extra data dict."
        ),
        help="Evolution task description",
    )
    parser.add_argument("--tool-name", default="hello_world_tool", help="Name of the tool to optimize")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
