...
This guide covers running AgentEvolver's MetaAgent on **ProgramBench** tasks and
...
```bash
conda create -n agentevolver python=3.12 && conda activate agentevolver
...
`OPENROUTER_API_KEY/BASE`, `ANTHROPIC_API_KEY/BASE` (see `agentevolver/model/context.py`).
...
```bash
python -c "from agentevolver.data.programbench import ProgramBenchDataset as D; \
...
- **Network isolation is ON by default** (`--network none`, the anti-cheat that stops the
  ...
  - The agent cannot fetch build dependencies inside the sandbox. Languages that pull deps at
    ...
    pre-caches them.
  - Disable only for debugging: `AGENTEVOLVER_SANDBOX_ISOLATE_NETWORK=0` (this permits the
...
```bash
conda activate agentevolver
IMG=$(python -c "from agentevolver.data.programbench import ProgramBenchDataset as D; \
