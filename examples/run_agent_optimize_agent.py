"""Run AgentOptimizeAgent to evolve a generated agent."""
import os
import sys
import json
import asyncio
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run AgentOptimizeAgent to evolve a generated agent")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "agent_optimize_agent.py"),
        help="Config file path",
    )
    parser.add_argument(
        "--task",
        default=(
            "Improve the summary_agent so that it also reports the number of words and "
            "characters in the source file alongside the summary. Include both counts in "
            "the returned AgentResponse message."
        ),
        help="Evolution task description",
    )
    parser.add_argument("--agent-name", default="summary_agent", help="Name of the agent to optimize")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
...
async def run_optimize_agent(record: TaskRecord):
    """TaskManager handler: executes the agent optimize agent for a given TaskRecord."""
    session_id = record.task.session_id or make_id()
    ctx = SessionContext(id=session_id)

    target_name = (record.task.metadata or {}).get("target_name", "summary_agent")
...
async def main():
    ...
    logger.info(f"| 📋 Submitting agent optimization task: target={target_name}")
