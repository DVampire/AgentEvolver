"""Run SkillOptimizeAgent to optimize a skill."""
import os
import sys
import json
import asyncio
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run SkillOptimizeAgent to optimize a skill")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "skill_optimize_agent.py"),
        help="Config file path",
    )
    parser.add_argument(
        "--task",
        default=(
            "Improve the hello_world_skill so that it explicitly handles three greeting locales: "
            "English, Chinese, and Japanese. Add a Locale Selection step to the Instructions "
            "section and update the Examples section with one example per locale."
        ),
        help="Optimization task description",
    )
    parser.add_argument("--skill-name", default="hello_world_skill", help="Name of the skill to optimize")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
...
async def run_optimize_agent(record: TaskRecord):
    """TaskManager handler: executes the skill optimize agent for a given TaskRecord."""
    ctx = SessionContext()
...
async def main():
    ...
    await skill_manager.initialize(skill_names=getattr(config, "skill_names", [args.skill_name]))
    ...
    await task_manager.initialize(work_dir=task_work_dir, handler=run_optimize_agent)
    await task_manager.start(num_workers=1)

    # --- Submit task ---
    task_text = args.task
    target_name = args.skill_name

    logger.info(f"| 📋 Submitting skill optimization task: target={target_name}")
    ...
    if record.task.status == TaskStatus.DONE:
        logger.info(f"| ✅ Skill optimization completed: {task_id}")
    else:
        logger.error(f"| ❌ Skill optimization ended with status {record.task.status}: {record.error}")
