"""Run GenerateAgent to create a new component of any type.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run GenerateAgent to create a new component")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "generate_agent.py"),
        help="Config file path",
    )
    add_task_args(parser, default_task_file=os.path.join(root, "examples", "tasks", "tool_generate.html"))
...
async def run_generate_agent(record: TaskRecord, ctx: SessionContext):
    """TaskManager handler: runs the generate agent for a given TaskRecord."""
...
async def main():
    ...
    await task_manager.initialize(log_root=task_log_root, handler=lambda record: run_generate_agent(record, ctx))
