"""Run AgentEvaluateAgent to evaluate a generated agent."""
import os
import sys
import json
import asyncio
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run AgentEvaluateAgent to evaluate a generated agent")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "agent_evaluate_agent.py"),
        help="Config file path",
    )
    parser.add_argument(
        "--task",
        default=(
            "Evaluate the summary_agent across all five quality dimensions "
            "(interface compliance, code quality, prompt quality, integration, task execution) "
            "and produce a structured report with per-dimension scores."
        ),
        help="Evaluation task description",
    )
    parser.add_argument("--agent-name", default="summary_agent", help="Name of the agent to evaluate")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
...
async def run_evaluate_agent(record: TaskRecord):
    """TaskManager handler: executes the agent evaluate agent for a given TaskRecord."""
...
async def main():
    ...
    logger.info(f"| 📋 Submitting agent evaluation task: target={target_name}")
