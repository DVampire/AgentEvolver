"""Run SkillGenerateAgent to generate a new skill."""
import os
import sys
import json
import asyncio
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run SkillGenerateAgent to generate a new skill")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "skill_generate_agent.py"),
        help="Config file path",
    )
    parser.add_argument(
        "--task",
        default=(
            "Generate a skill called 'code_review_skill' that guides an agent through "
            "performing a structured code review. The skill should cover: reading the diff, "
            "checking for correctness bugs, checking for style issues, and producing a "
            "structured review report with severity levels (critical/warning/suggestion)."
        ),
        help="Generation task description",
    )
    parser.add_argument("--skill-name", default="code_review_skill", help="Name for the skill to generate")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
...
async def run_generate_agent(record: TaskRecord):
    """TaskManager handler: executes the skill generate agent for a given TaskRecord."""
    ctx = SessionContext()
...
async def main():
    ...
    logger.info(f"| 📋 Submitting skill generation task: target={target_name}")
    ...
    if record.task.status == TaskStatus.DONE:
        logger.info(f"| ✅ Skill generation completed: {task_id}")
    else:
        logger.error(f"| ❌ Skill generation ended with status {record.task.status}: {record.error}")
