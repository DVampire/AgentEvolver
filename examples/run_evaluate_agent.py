"""Run EvaluateAgent to evaluate an existing component of any type.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run EvaluateAgent to evaluate an existing component")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "evaluate_agent.py"),
        help="Config file path",
    )
    add_task_args(parser, default_task_file=os.path.join(root, "examples", "tasks", "tool_evaluate.html"))
...
async def run_evaluate_agent(record: TaskRecord, ctx: SessionContext):
    """TaskManager handler: runs the evaluate agent for a given TaskRecord."""
