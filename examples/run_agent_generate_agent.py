"""Run AgentGenerateAgent to generate a new agent."""
import os
import sys
import json
import asyncio
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run AgentGenerateAgent to generate a new agent")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "agent_generate_agent.py"),
        help="Config file path",
    )
    parser.add_argument(
        "--task",
        default=(
            "Generate a workflow agent called 'summary_agent' that reads a text file, "
            "summarizes its content using the bash_tool to call a Python one-liner, "
            "and returns the summary as an AgentResponse."
        ),
        help="Generation task description",
    )
    parser.add_argument("--agent-name", default="summary_agent", help="Name for the agent to generate")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
...
async def run_generate_agent(record: TaskRecord):
    """TaskManager handler: executes the agent generate agent for a given TaskRecord."""
...
async def main():
    ...
    if record.task.status == TaskStatus.DONE:
        logger.info(f"| ✅ Agent generation completed: {task_id}")
