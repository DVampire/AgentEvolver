"""Run EnvironmentGenerateAgent to generate a new environment."""
import os
import sys
import json
import asyncio
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run EnvironmentGenerateAgent to generate a new environment")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "environment_generate_agent.py"),
        help="Config file path",
    )
    parser.add_argument(
        "--task",
        default=(
            "Generate an environment called 'counter_environment' that maintains an integer counter. "
            "Expose two actions: 'increment' (adds a given amount, default 1) and 'reset' (sets it to 0). "
            "get_state should report the current counter value."
        ),
        help="Generation task description",
    )
    parser.add_argument("--environment-name", default="counter_environment", help="Name for the environment to generate")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
...
async def run_generate_environment(record: TaskRecord):
    """TaskManager handler: executes the environment generate agent for a given TaskRecord."""
...
async def main():
    ...
    logger.info(f"| 📋 Submitting environment generation task: target={target_name}")
    ...
    if record.task.status == TaskStatus.DONE:
        logger.info(f"| ✅ Environment generation completed: {task_id}")
