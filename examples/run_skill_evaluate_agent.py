"""Run SkillEvaluateAgent to evaluate a skill."""
import os
import sys
import json
import asyncio
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run SkillEvaluateAgent to evaluate a skill")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "skill_evaluate_agent.py"),
        help="Config file path",
    )
    add_task_args(parser, default_task_file=os.path.join(root, "examples", "tasks", "skill_evaluate.html"))
    parser.add_argument("--skill-name", default="hello_world_skill", help="Name of the skill to evaluate")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
...
async def main():
    ...
    logger.info(f"| 📋 Submitting skill evaluation task: target={target_name}")
    ...
    if record.task.status == TaskStatus.DONE:
        logger.info(f"| ✅ Skill evaluation completed: {task_id}")
    else:
        logger.error(f"| ❌ Skill evaluation ended with status {record.task.status}: {record.error}")
