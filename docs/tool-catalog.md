...
| [`inspect_capability_tool`](#inspect_capability_tool) | `read_only` | no | `InspectCapability` | `agentevolver.tool.default.inspect_capability` |
...
Fetch one capability's full contract (instruction, call schema) plus its live registry facts — version, evolvability/enable_evolving, source paths. Types: tool, skill, connector, agent, environment, workflow, plugin.
