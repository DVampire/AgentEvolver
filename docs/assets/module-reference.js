/* Source-aligned detail for the module cards. The card already states the
 * short purpose; each entry adds runtime placement, boundary and public surface. */
(function () {
  const D = {
    agent:["Agent · MetaAgent · AgentConfig · AgentContext · agent_manager", "Agent 通过共享循环执行思考与行动。ContextAssembler 负责请求布局，ToolRouter 负责能力分发，Runtime Kernel 负责进程调度；Website 与 Game Builder 复用该循环。", "Agent owns the shared think–act loop. ContextAssembler owns request layout, ToolRouter dispatches capabilities, and the runtime kernel schedules processes. Website and Game Builders reuse this loop."],
    runtime:["Kernel · Process · RunBudget · Mailbox · Signal · kernel", "管理 Agent 进程、邮箱、信号、父子关系、主题订阅和生命周期清理。支持一次性任务和常驻订阅者；根进程与子进程共享运行预算。持久任务记录属于 task。", "Owns agent processes, mailboxes, signals, parent/child relationships, topic subscriptions and cleanup. Supports one-shot work and resident subscribers, with a shared root/descendant budget. Durable task records belong to task."],
    session:['SessionManagerServer · Session · SessionContext · BaseContext','定义一次调用共享的身份与项目状态。Manager 创建并读取 Session，Context 绑定 workspace 和 log root、暂存输入并写 manifest；Session 可包含 Conversation 和 Task，但不等于其中任何一个。','Owns invocation identity and project state. The manager creates and reads sessions; context binds workspace/log roots, stages inputs, and writes manifests.'],
    ...
    memory:["memory_manager · Memory · ProjectNotes", "为会话内的 Agent 提供持久笔记索引和可检索上下文。完整笔记按需读取，历史压缩由 ContextAssembler 管理；Trace 保存执行事件，Trajectory 提供训练投影。", "Provides per-actor, session-scoped note indexes and retrievable context. Note bodies are read on demand; ContextAssembler owns history compaction. Trace stores execution events and Trajectory projects training data."],
    ...
    telemetry:['OpenTelemetryTraceBridge','把权威 Trace 事件转换成可选的 OpenTelemetry Span。导出是尽力而为的投影：收集器或可选 SDK 故障不会改变 Agent 执行，持久事实仍由 Trace 保存。','Projects authoritative Trace events into optional OpenTelemetry spans. Export is best-effort: collector or SDK failures never change agent execution, and Trace remains the durable source of truth.'],
    ...
    environment:['EnvironmentManagerServer · Environment · EnvironmentConfig · ActionConfig · get_instruction','定义有状态的执行环境，并把每个 Action 暴露成模型能力。后续动作可依赖早先状态，因此创建、状态报告和清理都属于契约；这与通常无状态的 Tool 不同。调用统一返回 Response。','Defines stateful environments whose actions become model capabilities. Later actions may depend on earlier state, so creation, reporting, and cleanup are contract parts. Every call returns a Response.'],
    ...
    task:['TaskManagerServer · TaskRecord · TaskStatus · TaskPriority · GoalStore · GoalAuthority','建模持久工作与目标的优先级、状态、类别和 Authority。输入先规范化为 Task，Worker 领取 TaskRecord、执行 Handler 并落盘结果；Session 提供上下文，Runtime 推进 Agent。','Models durable work and goals. Workers claim records and persist results; Session supplies context and Runtime advances live agents.'],
    ...
    plan:["PlanManagerServer · PlanState · action_is_allowed", "维护 plan/index.md 摘要与 plan.md 详细计划，只将受大小限制的索引投影到实时上下文。额外记录由 Skill 建议、Agent 自定；显式 plan 模式还会在人工批准前限制行动。", "Maintains plan/index.md and plan.md, projecting only a bounded index into live context. Skills can suggest additional records; the agent chooses their structure. Explicit plan mode also restricts actions pending approval."],
    ...
    extension:["ExtensionManagerServer · Manifest · ManifestComponent · ComponentEvaluation", "在核心代码之外注册、归档和加载候选组件。隔离准入检查结构；自评报告绑定版本和真实调用证据，随后记录保留、回滚或卸载决策。按任务要求另行留存后续使用凭证。", "Registers, archives and loads candidates outside the core. Isolated admission checks structure; self-evaluation binds the candidate version to actual call evidence. Keep/rollback/unload and task-required consumer receipts are separate lifecycle steps."],
    ...
    gateway:['AgentGateway · GatewayCommand · GatewayResponse · GatewayEvent · create_websocket_app','是 CLI、TUI、Web UI 的统一版本化后端边界，并统一启停所有 Manager。客户端通过进程内、stdio 或 WebSocket 发送 Command，Gateway 校验 Session、分发并流式返回 Event。','The single versioned backend for CLI, TUI, and Web UI, owning manager lifecycle and command/event transport.'],
