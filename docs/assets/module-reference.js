...
(function () {
  const D = {
    ...
    runtime:['RuntimeManager · AgentRef · AgentStatus · TaskMessage · StopMessage','管理存活 Agent 的引用、邮箱与事件泵：协议消息投递到 AgentRef，Runtime 唤醒并推进目标 Agent，同时记录生命周期状态。它调度 live actor，不管理持久化用户任务。','Owns live agent references, mailboxes, and the event pump. It advances actors and records lifecycle state, while durable user-task records belong to task.'],
    ...
    job:['JobManagerServer · Job · JobStatus · ScheduleError · resolve_due','在后台运行长任务并保存到期提醒。启动立即返回 Job ID，Agent 稍后 inspect/wait/collect，把模型回合留给决策；Subagent 在它上面增加委派身份。','Runs long work and due reminders in the background. It returns an id immediately; Subagent adds delegated-agent identity on top.'],
    subagent:['SubagentServer · Subagent · ChildState','创建由后台 Job 承载的 Child Agent，使 Parent 无需阻塞；可持续 Child 还能接收 follow-up。它负责委派生命周期，Runtime 负责投递，Protocol 定义消息含义。','Runs child agents as background jobs and optionally keeps them continuable. Runtime delivers; Protocol defines message meaning.'],
